 ...
 * 미션3: 이름 기반 → 로그인 세션 기반으로 전환. 본인 검증은 로그인 사용자 기준.
 */
public class MyReservationStepTest extends IntegrationTest {
    ...
    void setUp() {
        ...
        brownId = helper.insertMember("brown@a.com", "password", "브라운", "USER");
        konId = helper.insertMember("kon@a.com", "password", "콘", "USER");
        brownCookie = helper.loginAndGetCookie("brown@a.com", "password");
    }
    ...
    class MyReservationList {
        ...
        void 내_예약_조회() {
            ...
            helper.insertReservation(konId, FUTURE_DATE_1, timeId10, themeId);  // 콘 예약(필터링 검증용)
    ...
    class MyReservationCancel {
        ...
        void 다른_사람의_예약() {
            Long reservationId = helper.insertReservationAndReturnId(konId, FUTURE_DATE_1, timeId10, themeId);

            // 브라운으로 로그인한 세션으로 콘의 예약 취소 시도
            RestAssured.given().log().all()
                    .cookie("JSESSIONID", brownCookie)
                    .when().delete("/user/reservations/" + reservationId)
                    .then().log().all()
    ...
    class MyReservationUpdate {
        ...
        void 시간_충돌() {
            ...
            helper.insertReservationAndReturnId(konId, FUTURE_DATE_1, timeId11, themeId);
